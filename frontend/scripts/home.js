(() => {
  ...
  function updateUI(tickets) {
    ...
    tickets.forEach((el) => {
      ...
      newTR.innerHTML = `<tr class="TicketRow" data-ticket="1"><td class="ticketStatus ticket--${el.status}">${el.status}</td><td>${el.priority}</td><td>${el.subject}</td><td>${el.reporter}</td><td>${el.agent}</td></tr>`;
      rows++;
    });
